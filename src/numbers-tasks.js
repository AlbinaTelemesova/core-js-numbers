...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthFirst = (x1 ** 2 + y1 ** 2) ** 0.5;
  const lengthSecond = (x2 ** 2 + y2 ** 2) ** 0.5;
  return Math.acos(scalarProduct / (lengthFirst * lengthSecond));
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    const newNum = previousNum + nextNum;
    previousNum = nextNum;
    nextNum = newNum;
  }
  return nextNum;
